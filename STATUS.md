...
- **现状评审 + P0/P2 行动（2026-08-17）**：`现状评审_2026-08-17.md`（工程✅/裁决⚠️未启动/纪律✅/外部依赖🔒）；`H-M1_主动外联包_2026-08-17.md`（外联实际发送待用户确认）；`最小真实数据集方案_2026-08-17.md`；README 文档分区（核心科学 vs 继承工程）+ 索引；PROJECT_PLAN L2「42/42→59/59」表述修正。
- **数据获取路径打通 + 首次真实数据（2026-08-17）**：hf-mirror 解锁 HF；实际下载 `lerobot/pusht`（MIT，206 episodes/25650 帧/96×96 av1）落盘云端——项目首次获取真实机器人数据；后续依赖=视频解码器（pip install av）+ 格式映射（96→384）；pusht 非厨房域（适合管线验证、不适合 H-T5）。
- **真实数据管线冒烟通过（2026-08-17）**：`scripts/lerobot_to_windows.py`（LeRobot→AC-VJEPA 窗口转换器）入库；pusht 15 episodes→206 窗口；P1 冻结骨干在真实 pusht 帧上训练 loss 单调下降（−0.21→−0.38）——真实（非合成）数据首次流过训练管线。
